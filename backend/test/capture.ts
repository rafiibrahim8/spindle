...
async function discover(): Promise<State> {
...
console.log(`discovered: track=${state.trackId} album=${state.albumId} artist=${state.artistId} ` +
            `smartPlaylist=${state.smartPlaylistId} genre="${state.genre}" art=${state.artFile.slice(0, 12)}…`);
...
await Bun.write(OUT, JSON.stringify({ discovered: state, records }, null, 2) + '\n');
const codes = records.reduce<Record<string, number>>((a, r) => { a[r.status] = (a[r.status] ?? 0) + 1; return a; }, {});
console.log(`captured ${records.length} steps -> ${OUT}`);
